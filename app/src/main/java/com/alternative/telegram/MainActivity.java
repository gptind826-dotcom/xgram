/*
 * ᴍᴀɪɴᴀᴄᴛɪᴠɪᴛʏ.ᴊᴀᴠᴀ — xɢʀᴀᴍ
 * ᴘᴏꜱᴛ-ᴀᴜᴛʜᴇɴᴛɪᴄᴀᴛɪᴏɴ ᴅᴀꜱʜʙᴏᴀʀᴅ
 *
 * ꜰᴇᴀᴛᴜʀᴇꜱ:
 * - ʟɪǫᴜɪᴅ ɢʟᴀꜱꜱ ɴᴀᴠɪɢᴀᴛɪᴏɴ
 * - ʀᴇᴀʟ ᴛᴇʟᴇɢʀᴀᴍ ᴀᴘɪ ɪɴᴛᴇɢʀᴀᴛɪᴏɴ (ʙᴏᴛ ɪɴꜰᴏ, ᴜᴘᴅᴀᴛᴇꜱ)
 * - ʙᴀᴄᴋɢʀᴏᴜɴᴅ ᴜʀʟ ꜱᴜᴘᴘᴏʀᴛ
 * - ᴄᴏɴᴅɪᴛɪᴏɴᴀʟ ꜱᴇᴛᴛɪɴɢꜱ ʙᴀꜱᴇᴅ ᴏɴ ʟᴏɢɪɴ ᴛʏᴘᴇ
 */
...
import com.alternative.telegram.api.TelegramApiClient;
...
public class MainActivity extends AppCompatActivity {
    ...
    private void loadTelegramData() {
        if (sessionManager.isBotLogin()) {
            loadBotData();
        } else if (sessionManager.isPhoneLogin()) {
            loadUserData();
        } else if (sessionManager.isSessionLogin()) {
            loadSessionData();
        }
    }
    ...
    private void loadBotData() {
        authManager.getBotInfo(new TelegramAuthManager.BotInfoCallback() {
            ...
            public void onError(String error) {
                Log.w(TAG, "Failed to load bot info: " + error);
        ...
        // Load recent updates
        authManager.getBotUpdates(0, new TelegramAuthManager.UpdatesCallback() {
            @Override
            public void onUpdatesReceived(JSONArray updates) {
                Log.i(TAG, "Received " + updates.length() + " bot updates");
            }

            @Override
            public void onError(String error) {
                Log.w(TAG, "Failed to load updates: " + error);
            }
        });
    }
    ...
    private void setupSettingsPanel() {
        ...
        if (sessionManager.isBotLogin()) {
            ...
            if (sessionManager.isPhoneLogin()) {
                settingsProfileStatus.setText(MiniFontConverter.convert(
                        sessionManager.getPhoneNumber() != null ? sessionManager.getPhoneNumber() : ""));
            } else if (sessionManager.isSessionLogin()) {
                settingsProfileStatus.setText(MiniFontConverter.convert(
                        "ᴅᴄ" + sessionManager.getDcId() + " | @" + username));
            } else {
                settingsProfileStatus.setText(MiniFontConverter.convert("@" + username));
            }
        }
    }
    ...
    private void performLogout() {
        authManager.logout(new TelegramAuthManager.LogoutCallback() {
            @Override
            ...
            public void onError(String error) {
                MiniFontConverter.showToast(MainActivity.this,
                        "ʟᴏɢᴏᴜᴛ ᴇʀʀᴏʀ: " + error, Toast.LENGTH_SHORT);
                navigateToLogin();
            }
        });
    }
