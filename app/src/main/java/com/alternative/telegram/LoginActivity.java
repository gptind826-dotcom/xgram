...
public class LoginActivity extends AppCompatActivity {
    ...
    private void setupBackPressHandler() {
        getOnBackPressedDispatcher().addCallback(this, new androidx.activity.OnBackPressedCallback(true) {
            ...
            public void handleOnBackPressed() {
                if (formOverlay != null && formOverlay.getVisibility() == View.VISIBLE) {
                    hideLoginForm();
                    return;
                }
                finish();
    ...
    private void showLoginForm(LoginMode mode) {
        ...
        switch (mode) {
            case PHONE:
                ...
                formDescription.setText(MiniFontConverter.convert("ᴇɴᴛᴇʀ ʏᴏᴜʀ ᴘʜᴏɴᴇ ɴᴜᴍʙᴇʀ ᴛᴏ ʀᴇᴄᴇɪᴠᴇ ᴀɴ ꜱᴍꜢ ᴄᴏᴅᴇ"));
    ...
    // ═══════════════════════════════════════════════════════════
    // ʟᴏɢɪɴ ʜᴀɴᴅʟᴇʀꜱ — ɴᴏᴡ ᴡɪᴛʜ ʀᴇᴀʟ ᴛᴇʟᴇɢʀᴀᴍ ᴀᴘɪ
    // ═══════════════════════════════════════════════════════════
    ...
    private void handlePhoneLogin() {
        ...
        showLoading(true);
        statusText.setText(MiniFontConverter.convert("ʀᴇǫᴜᴇꜱᴛɪɴɢ ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ ᴄᴏᴅᴇ..."));
        statusText.setVisibility(View.VISIBLE);

        // Use real Telegram auth manager to request code
        authManager.requestPhoneCode(countryCode, phoneNumber, DEFAULT_API_ID, DEFAULT_API_HASH,
                new TelegramAuthManager.PhoneCodeCallback() {
                    ...
                    public void onCodeSent(String type, int timeout) {
                        ...
                        intent.putExtra("phone_number", result.countryCode + result.phoneNumber);
                        intent.putExtra("country_code", result.countryCode);
                        intent.putExtra("phone", result.phoneNumber);
                        intent.putExtra("api_id", DEFAULT_API_ID);
                        intent.putExtra("api_hash", DEFAULT_API_HASH);
                    ...
                    public void onError(String error) {
                        showLoading(false);
                        showError("ꜰᴀɪʟᴇᴅ ᴛᴏ ꜱᴇɴᴅ ᴄᴏᴅᴇ: " + error);
                    }
                });
    }
    ...
    private void handleBotLogin() {
        ...
        showLoading(true);
        statusText.setText(MiniFontConverter.convert("ᴠᴀʟɪᴅᴀᴛɪɴɢ ʙᴏᴛ ᴛᴏᴋᴇɴ..."));
        statusText.setVisibility(View.VISIBLE);

        // Use real Telegram Bot API
        authManager.authenticateWithBotToken(token, new TelegramAuthManager.AuthCallback() {
            @Override
            public void onSuccess(TelegramAuthManager.AuthMethod method, long userId,
                                   String displayName, String username) {
                showLoading(false);
                statusText.setText(MiniFontConverter.convert("ʙᴏᴛ ᴄᴏɴɴᴇᴄᴛᴇᴅ: @" + username));
                statusText.setTextColor(getResources().getColor(R.color.glass_success, getTheme()));

                mainHandler.postDelayed(() -> navigateToMain(), 800);
            }

            @Override
            public void onError(String error) {
                showLoading(false);
                showError("ʙᴏᴛ ʟᴏɢɪɴ ꜰᴀɪʟᴇᴅ: " + error);
            }
        });
    }
    ...
    private void handleSessionLogin() {
        ...
        showLoading(true);
        statusText.setText(MiniFontConverter.convert("ᴘᴀʀꜱɪɴɢ ᴀɴᴅ ᴠᴀʟɪᴅᴀᴛɪɴɢ ꜱᴇꜱꜱɪᴏɴ..."));
        statusText.setVisibility(View.VISIBLE);

        // Use real session validation
        authManager.authenticateWithSessionString(sessionString, new TelegramAuthManager.AuthCallback() {
            @Override
            public void onSuccess(TelegramAuthManager.AuthMethod method, long userId,
                                   String displayName, String username) {
                showLoading(false);
                statusText.setText(MiniFontConverter.convert("ꜱᴇꜱꜱɪᴏɴ ᴠᴀʟɪᴅ! ᴄᴏɴɴᴇᴄᴛɪɴɢ..."));
                statusText.setTextColor(getResources().getColor(R.color.glass_success, getTheme()));
            ...
            public void onError(String error) {
                showLoading(false);
                showError("ꜱᴇꜱꜱɪᴏɴ ʟᴏɢɪɴ ꜰᴀɪʟᴇᴅ: " + error);
            }
        });
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == OTP_REQUEST_CODE && resultCode == RESULT_OK && data != null) {
            String code = data.getStringExtra("otp_code");
            String countryCode = data.getStringExtra("country_code");
            String phone = data.getStringExtra("phone");
            int apiId = data.getIntExtra("api_id", DEFAULT_API_ID);
            String apiHash = data.getStringExtra("api_hash");

            if (code != null && !code.isEmpty()) {
                // Complete phone authentication with the code
                showLoading(true);
                statusText.setText(MiniFontConverter.convert("ᴠᴇʀɪꜰʏɪɴɢ ᴄᴏᴅᴇ..."));
                statusText.setVisibility(View.VISIBLE);

                authManager.verifyPhoneCode(code, apiId, apiHash, new TelegramAuthManager.AuthCallback() {
                    @Override
                    public void onSuccess(TelegramAuthManager.AuthMethod method, long userId,
                                           String displayName, String username) {
                        showLoading(false);
                    ...
                    public void onError(String error) {
                        showLoading(false);
                        showError("ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ ꜰᴀɪʟᴇᴅ: " + error);
                    }
                });
            }
        }
